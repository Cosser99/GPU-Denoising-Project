...
namespace
{
    ...
    __global__ void makeTransferFunction(double* transferFunction, size_t paddedWidth, size_t paddedHeight, double sigma)
    {
        ...
        transferFunction[y * paddedWidth + x] = exp(-(dx * dx + dy * dy) / (2.0 * sigma * sigma));
    }
                                           ...
                                           size_t paddedWidth, size_t paddedHeight, size_t paddedSize)
    {
        ...
        paddedImage[static_cast<size_t>(channel) * paddedSize + y * paddedWidth + x] =
            make_cuDoubleComplex((x + y) % 2 == 0 ? value : -value, 0.0);
    }

    __global__ void multiplyTransferFunction(cufftDoubleComplex* paddedImage, const double* transferFunction,
    ...
    template <typename PixelT>
    __global__ void extractFilteredImages(const cufftDoubleComplex* paddedImage, PixelT* output,
                                          int width, int height, int nChannels,
...
namespace idl
{
    ...
    Image<PixelT> FFTFilter<PixelT>::apply(const Image<PixelT>& input) const
    {
        ...
        cudaMalloc(&devicePaddedImage, paddedSize * input.nChannels() * sizeof(cufftDoubleComplex));
        ...
        cufftCheckErrors(cufftPlanMany(&plan, 2, dimensions, nullptr, 1, static_cast<int>(paddedSize),
                                       nullptr, 1, static_cast<int>(paddedSize), CUFFT_Z2Z, input.nChannels()));
